...
- Bundled fonts: `assets/reading.ttf` (Inter subset, OFL) heads the
  ...
  drawable. `mathtext` hands the app RUNS (text + scale + rise), never a
  string: the ladder is what makes `e^{z_i}` an exponent with an index
  instead of `e^(zᵢ)`, leaning letters are already the Mathematical
  Alphanumeric characters (a renderer's italics flag only shears), and
  `navigator::math_job` is the ONE place that turns a run into a
  `line_height`-placed span. `navigator::reading_frame` is the ONE
  typography scope for markdown (pane + hover popup) and
  `markdown_viewer` the one renderer config.
