...
- Bundled fonts: `assets/reading.ttf` (Inter subset, OFL) heads the
  ...
  sat off the letters' baseline and inflated the row, dragging every
  subscript down. One family, one baseline; the fallbacks behind it must
  never fire. A missing glyph is a REPLACEMENT BOX, not nothing:
  math.ttf's codepoints come from `mathtext::glyphs()` via
  `assets/gen-math-font.sh` (never a hand-kept list), and
  `every_character_mathtext_can_draw_has_a_glyph` fails when the symbol
  tables outgrow the font. Never put U+25FB in those tables — epaint
  uses it as its replacement character, and a face holding it becomes
  the family's replacement face, after which nothing in it reports as
  drawable. `mathtext` hands the app a `Node` TREE, never a string or a
  line of runs: a fraction is numerator OVER denominator and a script is
  a base WITH something on it, because flattening either one showed
  (`a/b` needed parentheses to stay true, and a script had to be spelled
  with whatever character Unicode happened to have). `app/math.rs` is the
  ONE place that sets it — a box model with a `Frame` per node, and its
  module doc carries the rule that decides the hard cases: ascent and
  descent are the FONT's (baselines, line heights), the ink box is where
  the glyphs really are, and anything that has to touch what it draws (a
  radical's bar, a delimiter grown to its content) measures ink.
